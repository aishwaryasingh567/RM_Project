...
$edit_event = "SELECT * FROM `events` WHERE id = '".$_GET['event_id']."'";
$edit_event = mysqli_query($conn,$edit_event);
$edit_event = mysqli_fetch_object($edit_event);
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container">
        ...
        <form id="addeventform">
            ...
            <input type="text" name="event_name" value="<?=$edit_event->name?>" required>
            <label name="task_name">Event Coordinator:</label>
            <!-- <select style="width: 100%;" name="event_coordinator" id="dog-names">
                Options here 
            </select> -->
            <input type="text" name= "event_coordinator" value="<?=$edit_event->coordinator?>">
            <label style="margin-top: 10px;" for="action">Event Description:</label>
            <input type="text" name="event_description" value="<?=$edit_event->description?>" required>

            <label style="margin-top: 10px;" for="action">Event Form start date:</label>
            <input type="date" name="event_start" value="<?=$edit_event->registration_start?>" required>

            <label style="margin-top: 10px;" for="action">Event Form Last date:</label>
            <input type="date" name="event_end" value="<?=$edit_event->registration_end?>" required>

            <label style="margin-top: 10px;" for="action">Event date:</label>
            <input type="date" name="event_date" value="<?=$edit_event->date?>" required>

            <label style="margin-top: 10px;" for="action">Event Credit:</label>
            <input type="text" name="event_credit" value="<?=$edit_event->point?>" required>
